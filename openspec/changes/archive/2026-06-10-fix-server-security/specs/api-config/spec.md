...
服务端 MUST 只在 `Access-Control-Allow-Origin` 中响应当前请求来源属于 `ALLOWED_ORIGINS` 环境变量配置的白名单时才允许跨域。白名单以逗号分隔，支持多个来源。若请求来源不在白名单中，服务端 MUST 不返回 `Access-Control-Allow-Origin` header（浏览器会阻止跨域请求）。
...
#### Scenario: 未配置白名单时默认拒绝跨域
- **WHEN** `ALLOWED_ORIGINS` 环境变量未设置或为空字符串
- **AND** 任何跨域请求到达
- **THEN** 响应不包含 `Access-Control-Allow-Origin` header

### Requirement: 所有 HTTP 响应携带 CSP Header
服务端 MUST 在每个 HTTP 响应中附加 `Content-Security-Policy: default-src 'self'` header。这限制页面只能加载同域资源，作为 XSS 的第二道防线。
...
服务端 MUST 只通过 HTTP 暴露以下路径：
- `/` → `index.html`
- `/index.html` → `index.html`
- `/battle-map.html` → `battle-map.html`
- `/data-viz.html` → `data-viz.html`
- `/css/*` → `css/` 目录下的文件
- `/js/*` → `js/` 目录下的文件
- `/assets/*` → `assets/` 目录下的文件

任何其他路径（如 `/server.js`、`/.env`、`/data/battle-map.db`、`/prisma/schema.prisma`）MUST 返回 404 Not Found。
...
#### Scenario: 数据库文件被阻止
- **WHEN** 请求 `GET /data/battle-map.db`
- **THEN** 返回 HTTP 404 Not Found
...
**变更说明**: 此 Requirement 的行为不变，但读取逻辑需要与新增的 `data-auth-token` 读取保持一致（同一初始化流程中依次读取两个 data 属性）。
...
- **单元测试** (`tests/cors.test.js`)：模拟不同 Origin 的请求，验证 CORS 白名单逻辑
- **单元测试** (`tests/static-routes.test.js`)：模拟请求 `/server.js`、`.env` 等路径，验证返回 404
- **单元测试** (`tests/csp.test.js`)：验证所有响应携带 CSP header
- **集成测试** (`tests/integration/static-frontend.test.js`)：验证前端资源可正常加载、后端文件被阻止
