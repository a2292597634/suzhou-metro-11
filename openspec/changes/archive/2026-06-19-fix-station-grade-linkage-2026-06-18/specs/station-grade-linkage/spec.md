...
- 集成测试：`tests/integration/viz-data.test.js` 覆盖商业信息管理页内单站编辑与批量等级管理的互相同步。
- 集成测试：`tests/integration/home-dashboard.test.js` 覆盖首页从 `state.stations[].grade` 派生等级显示和筛选。
- 单元测试：`tests/render.test.js` 覆盖主作战图分级面板从 `state.stations[].grade` 派生等级名单。
- Mock：DOM、`saveData()`、`requestAnimationFrame`；不 mock 等级规范化工具。
