...
- 单元测试：`tests/shop-schema.test.js` 覆盖 `photo` 字段校验。
- 单元测试：`tests/viz.test.js` 覆盖照片导入、替换、删除、类型/大小拒绝和悬停预览 DOM。
- 集成测试：`tests/integration/viz-data.test.js` 覆盖照片操作写入 `state.stations[].shops[].photo` 并通过 `saveData()` 保存。
- API 测试：`tests/integration/auth-data-flow.test.js` 覆盖 `/api/data` 保存和读取照片字段成功往返。
- 安全测试：`tests/server-security.test.js` 覆盖 `/api/data` 拒绝非法照片字段和超长照片字段。
