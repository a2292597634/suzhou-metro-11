...
`POST /api/login` 端点 MUST 接收 `{ password }` JSON body。若 `password` 等于 `process.env.AUTH_TOKEN`，MUST 设置名为 `auth_token` 的签名 HttpOnly Cookie（`httpOnly: true, signed: true, sameSite: 'lax', maxAge: 86400000`）并返回 `{ success: true }`。若密码不匹配，MUST 返回 HTTP 401 `{ error: '密码错误' }`。
...
### ~~Requirement: state.js 从 HTML 读取 Token~~（已删除）
`state.authToken` 从 `document.documentElement.dataset.authToken` 读取的方案已被移除。此方案将服务端密钥泄露到浏览器可访问的 HTML 中，存在严重安全隐患。

### ~~Requirement: 前端请求自动携带认证 Token~~（已删除）
`saveData()` 手动设置 `Authorization: Bearer <token>` 的方案已被移除。替换为 Cookie 自动携带方案。
